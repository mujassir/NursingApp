    ...
    public class BSImportData
    {
        ...
        public bool CheckImportDataFileStandard(string filePath)
        {
            ...
            try
            {
                ...
                while (index < _rows.Length - 1 && IsRowStatUMTHeader(row))
                {
                    ++index;
                    row = _rows[index];
                }
        ...
        /// <summary>
        /// 2011-0222 [amelinc] From Nelson Lee
        /// I just got off the phone with the software developer for the PDA’s.  He told me that they added these rows to the latest software to store 
        /// customized headers for the reports in StatUMT software.  
        /// He told me that we need to ignore any record in which the first field is less than or equal to 100.
        /// This should only be in the top of the file.  In the existing data, the first field is ‘7’.
        /// ----------
        /// This function is used to verify if the row is part of the header and to skip it
        /// </summary>
        /// <param name="row"></param>
        /// <returns></returns>
        private bool IsRowStatUMTHeader(string row)
        {
            string[] cols = row.Split('\t');
            if (cols.Length < 5)
            {
                if (Convert.ToInt32(cols[0]) < 100)
                {
                    return true;
                }
            }
